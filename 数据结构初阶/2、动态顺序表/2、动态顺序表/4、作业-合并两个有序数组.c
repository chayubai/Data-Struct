...
int main()
{
	int nums1[1024] = { 1,2,3};//nums1数组的空间足够大
...
//方法三、由于两个数都是升序排列(一个升序，一个倒序也可以)。nums1和nums2都从后往前走，取大的从后往前放
//如果num2中下标位置的元素大于nums1中下标位置的元素，则把大的元素放入num1数组所在的最后一个位置
//依次如此，如果nums2中的数据走完了，则结束。如果nums1没走完，而nums2走完了，无需处理，直接结束。
