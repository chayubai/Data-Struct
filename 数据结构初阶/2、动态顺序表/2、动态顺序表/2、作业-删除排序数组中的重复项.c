...
//如果这两个数不相同，两个指针都++
//如果重复的项太多，此方法时间复杂度较大：F(N) = n-1 + n-2 + ... + 1 = O(N^2)
//空间复杂度为O(1)
/*
#include <stdio.h>
...
int removeDuplicates(int* nums, int numsSize)
{
	...
	while(start2<=numsSize-1)
    {
        ...
        if(nums[start1]!=nums[start2])
        {
            ...
            {
                tmp[i] = nums[start1];//由于start1++;所以nums[start1]为最后一个数据
            }
        }
        else
        {
...
int removeDuplicates(int* nums, int numsSize)
{
	...
	//将最后一次的两种情况进行合并
	//此时cur = numsSize，prev = numsSize-1
	nums[dest] = nums[prev];
...
//方法4、在上面的基础上，去重 - 将第二个指针指向的数据与第一个指针不同时，更新第一个指针指向为第二个指针指向的数据
