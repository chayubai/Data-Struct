...
//
//	//方法二：
//	s.a = (SLDataType*)malloc(sizeof(SLDataType) * 4);//连续开辟四个SLDataType类型的空间大小
//	if (s.a == NULL)
//	{
//		printf("申请内存失败\n");
//		exit(-1);//结束程序
...
//	//方法三：
//	//静态顺序表的初始化
//	memest(s.a, 0, sizeof(SLDataType)* 4);//对四个元素的数组初始化
//	s.size = 0;
//	*/
//}
...
//		//不能直接开辟空间，否则ps->capacity *= 2;相当于capacity  = 0 * 2 = 0
...
//	//对于删除数据本质没有删除真正删除。故，一般删数据，只需要--即可。即不需要这一行ps->a[ps->size - 1] = 0;
//	//方法二：
//	//ps->a[ps->size - 1] = 0;
//	if(ps->size >= 0)
//	{
...
//{
//	assert(ps->size > 0);
//	//将第一个数据置为0，，从第二个数据开始，依次将每个数据往前移动
//	int start = 0;//定义一个指向最后一个空间的位置
...
//	}
//	/*
//	int start = 1;//定义一个指向最后一个空间的位置
//	while (start < ps->size)//注意条件，即start <= ps->size-2
//	{
...
void SeqListInsert(SL* ps, int pos, SLDataType x)
{
	...
	assert(ps);//为真不执行
	assert(pos <= ps->size && pos >= 0);//保持位置是有效性，即0 <= pos <= size
