...
//方法2、如果不限空间复杂度，可以：
//开辟一块空间和数组相同大小的空间，遍历nums数组中的元素，将不是val的元素放入开辟的空间中
//然后将空间的元素拷贝到原数组中将其覆盖，最后将size大小置为：原始数组元素个数 - val
//此时，Fn = 2N,时间复杂度为O(N),空间复杂度为O(N)
/*
#include <stdio.h>
int removeElement(int* nums, int numsSize, int val)
{
...
//方法3、把src找到不是val的数，往dest上放。使用两个下标一开始都同时指向第一个元素，
