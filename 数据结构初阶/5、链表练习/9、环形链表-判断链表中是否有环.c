...
int hasCycle(struct ListNode* head)
{
	...
	while (fast != NULL && fast->next != NULL)//循环条件：可能不为环，此时奇数个结点fast为空，偶数个结点fast->next为空结束
	{
		slow = slow->next;
		fast = fast->next->next;
		if (slow == fast)
