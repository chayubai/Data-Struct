...
//方法1、将相遇点断开，断开的结点，一个作为头结点，一个置为NULL。再转换成相交链表问题，求两个链表相交点。
//链表相交实现，详见作业8.
//注意：前提链表能够被破坏。

//方法2、慢指针进入环以后，慢指针在一圈内，快指针一定会追上慢指针
//结论：一个指针从相遇点开始走，一个指针从链表头开始走，它们会在环的入口点相遇。

//证明：
//假设头结点到循环结点距离为L，环的周长为C，入口节点到相遇节点的距离为X
//当两个指针相遇，
//则，此时，慢指针走了：L+X（慢指针最多走一圈）
//则，此时，快指针走了：L+X+n*C（n圈）,当L越大，快指针在环内转的圈越多
//由于快指针的速度是慢指针的两倍，同步走(时间相同)则二者的路程关系也为两倍
//(L+X)*2 = L+X+n*C，即L = n*C-X = (n-1)*C + C-X  (n-1)*C相当于从相遇点走回到相遇点，相当于没走，L = C-X
