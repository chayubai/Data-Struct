...
//方法三：从头开始，依次比较链表中的结点，每次取两个链表小的结点，尾插到新链表中
//1、无哨兵位的头结点
...
//优化2：带哨兵位的头结点(不存储有效数据)，此时无需判断头结点，直接取小的结点尾插
...
struct ListNode* mergeTwoLists(struct ListNode* L1, struct ListNode* L2)
{
	...
	head = tail = (struct ListNode*)malloc(sizeof(struct ListNode));//哨兵位的头结点，不存有效数据
