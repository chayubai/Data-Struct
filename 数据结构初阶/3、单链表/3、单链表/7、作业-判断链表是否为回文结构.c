...
//方法1：开一个900的数组，遍历链表将数据放入数组中，判断左边下标和右边下标对应的元素是否相等。
//数组回文结构的判断。

//方法2：查找中间结点，对相同部分的后半段进行逆置，再遍历前半段和后半段比较，判断是否相同。
//注意：查找中间结点的函数(在作业3中已经实现了)。反转链表(链表的逆置，在作业2中已经实现了)。
//对于中间结点的前一个结点的next需要置空，这样方便利用前一部分循环判断与后一部分是否一致。
//也可以不置空，利用后一部分循环判断与前一部分是否一致。
//					      prev
//                             slow           fast
